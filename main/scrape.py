...
def getClassAverage(self, classNID):
    classData = requests.get('https://'+self.serverAddress+'/core/node.json/'+str(classNID)+'?xds=MyWork&student='+str(self.studentData['unid']),cookies=self.getCookies(),headers=self.getHeaders()).json()['slices'][0]['data']
...
def getClassPlan(self, classNID):
    return requests.get('https://'+self.serverAddress+'/core/node.json/'+str(classNID)+'?xds=Course&_context=1',cookies=self.getCookies(),headers=self.getHeaders()).json()['slices'][0]['data']['col1']['outline']['plan']['tree']
...
def getCalendarData(self, date=date.today().strftime("%Y-%m-%d")):
    return requests.get('https://'+self.serverAddress+'/core/node.json/'+str(self.studentData['unid'])+'?xds=Calendar&targetDate='+str(date),cookies=self.getCookies(),headers=self.getHeaders()).json()["slices"][0]["data"]["caldata"]
...
def sendDirectMessage(self, message):
    ...
    return requests.post('https://'+self.serverAddress+'/core/create/'+str(message['to'])+'?xds=MessagesCompose&permaLinkKey=false&scopeState=true&_processed=true',data=payload,cookies=self.getCookies(),headers=self.getHeaders()).json() 